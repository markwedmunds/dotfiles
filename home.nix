...

{
  ...
  home.packages = with pkgs; [
    ...
    coreutils # GNU ls/dircolors so LS_COLORS below works (macOS BSD ls ignores it)
    ripgrep   # fast search
    fd        # fast find
    fzf       # fuzzy finder
    ...
    lazygit
    neovim
    helix     # modal editor; config lives in ~/.config/helix (symlinked below)
    tree-sitter  # CLI nvim-treesitter needs to generate the swift parser from grammar
    nodejs       # tree-sitter generate runs the swift grammar's grammar.js under node
    xcbeautify   # pretty xcodebuild logs for xcodebuild.nvim
    # language servers driving nvim's native LSP completion
    lua-language-server  # lua_ls: my neovim config
    nixd                 # nixd: this flake and home-manager config
    # flutter version manager; the SDKs it installs live in ~/fvm/versions
    fvm
  ...
  programs.zsh = {
    ...
    initContent = ''
      ...
      # Xcode build/run/test helpers (xb/xr/xt/xboot) for the Helix workflow.
      source ${dotfiles}/home/.config/zsh/xcode.zsh

      # Prefer a project-local fvm SDK when present so flutter/dart resolve to the
      # version pinned in .fvm/flutter_sdk. Re-evaluated on every cd via chpwd:
      # strip any previously-added project SDK, then prepend the current one.
      _fvm_use_project() {
        local clean="" p
        for p in ''${(s/:/)PATH}; do
          case $p in
            */.fvm/flutter_sdk/bin|*/.fvm/flutter_sdk/bin/cache/dart-sdk/bin) ;;
            *) clean="''${clean:+$clean:}$p" ;;
          esac
        done
        PATH="$clean"
        if [ -d "$PWD/.fvm/flutter_sdk/bin" ]; then
          export PATH="$PWD/.fvm/flutter_sdk/bin:$PWD/.fvm/flutter_sdk/bin/cache/dart-sdk/bin:$PATH"
        fi
      }
      autoload -Uz add-zsh-hook
      add-zsh-hook chpwd _fvm_use_project
      _fvm_use_project  # run once for the shell's starting directory

      # Nord dircolors (needs GNU ls from coreutils; matches old ~/.zshrc.hm-backup)
    ...
    shellAliases = {
      ...
      pull = "git pull";
      m = "git switch main";
      fl = "fvm flutter";  # fl run / fl test / fl devices, honouring the project's pinned SDK
